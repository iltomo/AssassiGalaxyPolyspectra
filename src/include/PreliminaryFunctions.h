...
class WindowFunctions:public GrowthFactor {
	...
	public:
		...
		WindowFunctions (std::string pfile, int ell, double sc, double c, double cav);
...
class GalaxyClustering:public WindowFunctions {

	//private:
	public:

		struct WindowsParameters WP = {sigmaChi, Chiav, DC, l, fC};
	...
	//public:
		...
		GalaxyClustering (std::string pfile, int ell, double sc, double c, double cav);

		double Wgalaxy (int l, double Chi, double aChi, double sigmaChi, int nD, int nG, int nRSD);


		void intWgalaxy (int l, std::complex <double> nu, double Chi1av, double sigmaChi1, double Chi2av, double sigmaChi2, int nRSD1, int nRSD2, double t, std::string GLQ, std::complex <double>& result);
